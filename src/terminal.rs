...
impl AddTerminalCommand for App {
    fn add_command(&mut self, command_impl: impl Command + 'static) -> &mut Self {
        let mut registry = self.world.get_resource_or_insert_with::<CommandRegistry>(Default::default);
...
fn send_raw_event(
	mut ev_writer: EventWriter<RawTerminalCommand>,
) {
...
impl Command for SendCommand {
    fn run_command(&mut self, args: &str, world: &mut World) {
		...
		connection.send_message::<UnorderedReliableChannel, TextMessage>(&TextMessage(String::from(args))).unwrap();
    }
...
impl Command for HostCommand {
    ...
    fn run_command(&mut self, _args: &str, world: &mut World) {
		world.insert_resource(NextState::<server::NetworkingState>(Some(server::NetworkingState::Started)))
    }
...
impl Command for ConnectCommand {
    ...
    fn run_command(&mut self, _args: &str, world: &mut World) {
		world.insert_resource(NextState::<client::NetworkingState>(Some(client::NetworkingState::Connecting)));
    }
...
fn process_raw_events(
	...
	world.resource_scope::<Events<RawTerminalCommand>,()>(|world, events| {
		for raw in ev_reader.read(&events) {
			...
			let mut command_registry: Mut<'static, CommandRegistry> = unsafe { std::mem::transmute(world.resource_mut::<CommandRegistry>()) }; 

			let Some(command) = command_registry.get_mut(command) else {
				error!("Command \"{}\" not found.\nType \"help\" to list all commands", command);
				return;
			};
